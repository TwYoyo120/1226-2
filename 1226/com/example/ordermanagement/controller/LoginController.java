...
import com.example.ordermanagement.service.LoginService;
...
public class LoginController {
    ...
    public String showLoginPageAdmin(HttpSession session) {
    	// 檢查 session 中是否有 user 資料，表示用戶已經登入
    	if(session.getAttribute("user") != null) {
    		// 如果已經登入，重定向到首頁（或其他頁面）
 ...
 // 顯示會員中心頁面
    ...
    public String showMemberCenterPage(HttpSession session) {
        if (session.getAttribute("user") == null) {
